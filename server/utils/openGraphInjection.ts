 ...
 */
function formatCompanyNameForUrl(companyName: string | null, fallbackId: string): string {
 ...
 */
export async function fetchCompanyForOpenGraph(slug: string): Promise<CompanyOpenGraphData | null> {
  try {
    ...
    if (!company) {
      ...
      company = allCompanies.find(c => formatCompanyNameForUrl(c.name, c.id.toString()) === slug) || null;
    }
 ...
 */
export async function fetchUserForOpenGraph(username: string): Promise<UserOpenGraphData | null> {
  try {
 ...
 */
export function injectOpenGraphTags(htmlTemplate: string, ogData: OpenGraphData): string {
  let modifiedHtml = htmlTemplate;
  
  // Replace title - handle cases where title already includes "| Sarasota Tech"
  const titleContent = ogData.title.includes("| Sarasota Tech") ? ogData.title : `${ogData.title} | Sarasota Tech`;
